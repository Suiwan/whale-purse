...
function parseBaseUrl(raw) {
  ...
  try {
    ...
    throw new Error(`whale-watch: invalid baseUrl "${raw}"`)
  }
  if (url.protocol !== 'https:' && url.protocol !== 'http:') {
    throw new Error(`whale-watch: baseUrl must be http(s), got "${url.protocol}"`)
  }
...
export function apply(ctx, config = {}) {
  ...
  if (typeof ctx.effect === 'function') ctx.effect(() => disposeRoutes, 'whale-watch: routes')
