...
/** 把 provider/model id 归一化到 flash/pro；无法识别返回 undefined。 */
...
class BalanceService {
  ...
  /** auto 模式下把模型 id 归一化；显式 pro/flash 时始终返回该模型。 */
  modelOf(modelId) {
    if (this.model === 'pro' || this.model === 'flash') return this.model
    return normalizeModel(modelId) ?? 'flash'
  }
    ...
    if (Array.isArray(events)) {
      for (let i = events.length - 1; i >= 0; i--) {
        ...
        if (event?.type === 'request/header') {
          const model = normalizeModel(event.data?.header?.config?.model)
  ...
  finalizedUsage(session) {
    ...
    for (const event of events) {
      if (event.type === 'request/header') {
        currentModel = this.modelOf(event.data?.header?.config?.model)
        continue
      }
  ...
  sessionCost(session) {
    ...
    const currentPrices = this.pricesFor(sessionModel, Date.now())
